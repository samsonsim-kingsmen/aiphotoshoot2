...
// ✅ Import media first before referencing
...
const DualResultView = ({
  ...
  // ❌ Removed `portal-animation` to kill the pink glow
  const capsuleBaseClasses =
    "w-full h-full rounded-full bg-cover bg-center transition-all duration-300 ease-in-out border-4 border-white/60 overflow-hidden relative";
  ...
  return (
    <div className="w-full h-screen relative overflow-hidden bg-black">
      ...
      >
        {/* Title PNG */}
        <div className="absolute top-5 left-0 right-0 z-20 flex justify-center transform -translate-y-[15%]">
          <img src={titleImg} className="transform scale-[0.8]" alt="Title" />
        </div>
        ...
        <div className="relative w-full h-full flex items-center justify-center">
          ...
          >
            <div className="absolute top-1/2 -left-12 -translate-y-1/2">
              <VerticalText
                text={retroTheme}
                className="text-white font-orbitron text-lg opacity-80"
              />
            </div>

            {/* Capsule only – ❌ removed portal-glow-bg */}
        ...
        <div className="absolute bottom-1 right-4 z-30">
          <img
            src={logoImg}
            alt="Logo"
            className="scale-[0.2] origin-bottom-right"
          />
        </div>
      </div>

      {/* Download button overlay – in front, ~2.5% from bottom */}
